#!/usr/bin/env python3
"""
List all tables for all WordPress sites from the discovery API.
No filtering by environment (prd/stg/dev).
"""
...
def get_all_sites_unfiltered():
    """Get ALL sites from discovery API without any filtering."""
    ...
    try:
        ...
        # Return ALL site names without filtering
        sites = []
...
def list_tables_for_all_sites():
    """List all tables for each site."""

    # Load config using config_loader (resolves ${ENV_VAR} patterns)
    config = load_config('wordpress')

    # Get ALL sites (no filtering)
    sites = get_all_sites_unfiltered()
    print(f"Found {len(sites)} total sites from API (unfiltered)")
    print()
    ...
    print()
    print("=" * 80)
    print("TABLE COUNTS ACROSS ALL SITES (sorted by frequency)")
    print("=" * 80)
    ...
    if failed_sites:
        print()
        print("Failed sites:")
        for f in failed_sites[:10]:  # Show first 10
