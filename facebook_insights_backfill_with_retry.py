#!/usr/bin/env python3
"""
Facebook Insights Backfill with Automatic Retry.

Runs month-sized Facebook page/post insight backfills sequentially, with durable
state, per-batch child logs, and retry of page-specific transient failures.
"""
...
CHILD_LOG_DIR = LOG_DIR / "facebook_backfill_batches"
WRAPPER_LOG_FILE = LOG_DIR / "facebook_backfill_wrapper.log"
DEFAULT_STATE_FILE = LOG_DIR / "facebook_insights_backfill_state.json"
...
TABLES = ("posts", "post_insights")
PAGE_ID_RE = re.compile(r"\bID:\s*(\d+)\b")
...
def configure_logging() -> logging.Logger:
    ...
    logger = logging.getLogger("facebook_backfill")
...
class FailedPageParser:
    """Incrementally parse page IDs from orchestrator retry/failure sections."""
    ...
    def feed(self, line: str) -> None:
        if "AUTOMATIC RETRY FOR FAILED PAGES" in line or "Transient API Errors" in line:
            self.in_failed_section = True
            return

        match = PAGE_ID_RE.search(line)
        if match and (self.in_failed_section or "Page:" in line):
            self.failed_pages.add(match.group(1))

        if self.in_failed_section and line.count("=") >= 20:
            self.in_failed_section = False
...
def generate_month_batches(start_date: str, end_date: str) -> List[Tuple[str, str]]:
    ...
    # Reverse so newest months are processed first (most recent data lands first)
    batches.reverse()
    return batches
...
def child_log_path(start_date: str, end_date: str, retry_attempt: int) -> Path:
    ...
    return CHILD_LOG_DIR / f"facebook_insights_{suffix}"


def build_command(start_date: str, end_date: str, sites: Optional[str], extra_args: Iterable[str]) -> List[str]:
    cmd = [
        sys.executable,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Facebook insights monthly backfill with retry and resume state.")
    parser.add_argument("--start-date", default=DEFAULT_START_DATE, help="Backfill start date, YYYY-MM-DD.")
    ...
    parser.add_argument("--max-retries", type=int, default=DEFAULT_MAX_RETRIES, help="Maximum attempts per failed page batch.")
...
def main() -> int:
    ...
    logger.info("=" * 80)
    logger.info("Facebook Insights Backfill with Automatic Retry")
    ...
    for batch_num, (start_date, end_date) in enumerate(batches, 1):
        ...
        if result.get("lock_contention"):
            ...
            logger.error("  View active locks with: %s", subprocess.list2cmdline([sys.executable, str(REPO_ROOT / "orchestrate.py"), "--env", "prod", "--job-locks"]))
        ...
        process_failed_without_page_list = not result["success"] and not failed_pages

        retry_attempt = 2
        while failed_pages and retry_attempt <= args.max_retries:
            logger.warning(
                "\n[Batch %d retry %d] %d page(s) failed, retrying...",
                batch_num,
                retry_attempt - 1,
                len(failed_pages),
            )
            ...
            new_failed_pages = list(retry_result["failed_pages"])
            recovered = len(failed_pages) - len(new_failed_pages)
            if recovered > 0:
                logger.info("[OK] Recovered %d page(s) on retry %d", recovered, retry_attempt - 1)
                total_retries += recovered

            if not retry_result["success"] and not new_failed_pages:
                process_failed_without_page_list = True
                result = retry_result
                break
        ...
        if process_failed_without_page_list:
            logger.error("[FAILED] Batch %d failed before page-specific retry information could be parsed", batch_num)
            logger.error("  Manual reprocessing command:")
            logger.error(
                "  %s",
                subprocess.list2cmdline(build_command(start_date, end_date, None, args.orchestrate_arg)),
            )
            failed_batches += 1
            state["failed"][key] = {
                "completed_at": now,
            ...
            logger.error("[FAILED] Batch %d has %d permanent page failure(s) after %d retries", batch_num, len(failed_pages), args.max_retries)
            logger.error("  Failed pages: %s", ",".join(failed_pages))
            logger.error("  Manual reprocessing command:")
            logger.error(
                "  %s",
                subprocess.list2cmdline(build_command(start_date, end_date, ",".join(failed_pages), args.orchestrate_arg)),
            )
            failed_batches += 1
            state["failed"][key] = {
                "completed_at": now,
    ...
    logger.info("Total pages recovered via wrapper retry: %d", total_retries)
